...
function ensure(value: unknown): asserts value { if (!value) throw new AgentRunError("AGENT_MODEL_SMOKE_FACT_MISMATCH"); }
...
export async function runSyntheticCompatibleSmoke(config: ModelConfig, fetcher: typeof fetch = fetch, mode: "basic" | "full" = "full") {
  ...
  const setup = () => {
    ...
    const raw = createChatCompletionsTransport(cfg, fetcher, budget);
    const transport: typeof raw = { async create(request, opts) { assertModelEgress("external", syntheticEgress); const result = await raw.create(request, opts); track(result); return result; } };
    return {
  ...
  for (const stage of ["jsonOutput", "strictStructuredOutput"] as const) {
    ...
    await probe(stage, async () => {
      if (stage === "jsonOutput") {
        ...
        const result = await callStructuredAgentIntent({ provider: adapterFactory.model, messages: createAgentIntentMessages("看看贸易站 1 的情况"), egress: syntheticEgress, timeoutMs: 15000, maxOutputTokens: 512 });
  ...
  for (const scenario of ["current", "saved"] as const) {
    ...
    try {
      ...
      if (scenario === "current") {
        ...
        ensure(record(record(summary.result).data).shiftCount === 2);
        ...
        ensure(record(detail.room).roomId === "trade_1" && shift.requestedShiftIndex === null && shift.resolvedShiftIndex === context.snapshot.activeShift && shift.usedActiveShift === true);
        ensure(planned.status === "available" && Array.isArray(planned.operators) && planned.operators.includes("贸易甲"));
        ensure(observed.status === "unavailable" && record(observed.issue).reason === "NO_OBSERVED_SNAPSHOT");
        ...
        ensure(forbidden.status === "missing" && record(forbidden.issue).code === "PLAN_NOT_FOUND_OR_FORBIDDEN" && !Object.hasOwn(forbidden, "data"));
      }
