...
import { createSavedPlanComparisonReadService, type SavedPlanComparisonRow } from "../saved-plan-comparison-service.ts";

export const savedPlanTestActor = { userId: "synthetic-owner" };
export const savedPlanTestNow = new Date("2026-09-07T00:00:00.000Z");
export function syntheticSavedPlan(id: string, userId = savedPlanTestActor.userId): SavedPlanComparisonRow & {
  publicResult: PublicPlanData; calculationContext: SavedPlanCalculationContext;
} {
  return {
    id, userId, title: `合成方案 ${id}`, pinned: false, diagnosticId: `synthetic-${id}`,
    createdAt: savedPlanTestNow, updatedAt: savedPlanTestNow, expiresAt: null,
    ...
    publicResult: {
      diagnosticId: `synthetic-${id}`, durationMs: 12345,
...
export function savedPlanComparisonTestContext(rows = [syntheticSavedPlan("left"), syntheticSavedPlan("right")]) {
  const metadata = createSavedPlanReadService({ requireConsent: async () => {}, loadMetadata: async () => rows, now: () => savedPlanTestNow });
  return { actor: savedPlanTestActor, service: createSavedPlanComparisonReadService({
    metadata, loadDetails: async () => rows, now: () => savedPlanTestNow,
  }) };
}
