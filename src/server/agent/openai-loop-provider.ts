...
export class OpenAILoopProvider implements LoopProvider {
  ...
  async next(request: LoopRequest) {
    ...
    try {
      ...
      if (this.initial !== undefined && this.initial !== initial) throw new AgentRunError("AGENT_RESPONSES_HISTORY_INVALID");
      this.initial = initial;
      if (!this.history.length) this.history = [
        { role: "system", content: READ_ONLY_ADVISOR_INSTRUCTIONS },
      ...
      for (const observation of request.observations.slice(this.consumed.length)) {
        if (this.pending.get(observation.call.id) !== JSON.stringify(observation.call)) throw new AgentRunError("AGENT_RESPONSES_HISTORY_INVALID");
      ...
      const calls = response.output.filter((item) => item.type === "function_call").map((item) => {
        ...
        record(args);
        const call = { id: item.call_id, name, arguments: args };
        this.seen.add(item.call_id); this.pending.set(item.call_id, JSON.stringify(call));
        return call;
      });
      ...
      if (decision.type === "final") { this.closed = true; this.history = []; this.consumed = []; this.pending.clear(); }
      return { decision, usage: response.usage };
    } catch (error) { this.closed = true; this.history = []; this.consumed = []; this.pending.clear(); throw error; }
