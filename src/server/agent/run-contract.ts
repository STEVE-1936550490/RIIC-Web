...
export type AgentSource = { type: "current_context" | "saved_plan" | "observed_schedule"; contextRevision: string | null; planDiagnosticId: string | null; sampledAt: string | null; updatedAt: string | null; planId: string | null };
...
export function parseAgentFinalResult(value: unknown): AgentFinalResult {
  ...
  const r = exact(value, ["status", "answer", "runId", "contextRevision", "modelMode", "intent", "sources", "limitations", "tools", "usage", "error"]);
  ...
  return { status: r.status, modelMode: r.modelMode, intent: null, answer: text(r.answer, 3000), runId: text(r.runId, 80), contextRevision: nullable(r.contextRevision, 80), error: nullable(r.error, 80),
    sources: list(r.sources, 60).map((raw): AgentSource => { const s = exact(raw, ["type", "contextRevision", "planDiagnosticId", "sampledAt", "updatedAt", "planId"]);
      if (s.type !== "current_context" && s.type !== "saved_plan" && s.type !== "observed_schedule") throw new AgentRunError("AGENT_INVALID_OUTPUT");
