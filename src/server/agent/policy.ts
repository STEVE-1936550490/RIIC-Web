...
export function canUseTool(name: string, context: AgentExecutionContext): boolean {
  if (name === "knowledge.get_skill_context") return isIssuedActor(context.actor) && context.knowledge !== undefined;
...
export function requireToolPolicy(name: string, context: AgentExecutionContext): void {
  if (name !== "knowledge.get_skill_context" && name !== "plan.preview" && !READ_ONLY_TOOL_NAMES.some((item) => item === name)) throw new AgentRunError("AGENT_UNKNOWN_TOOL");
